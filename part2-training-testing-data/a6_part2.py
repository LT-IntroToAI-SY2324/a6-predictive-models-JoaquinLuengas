...
coaf = round(float(model.coef_), 2)
intercept = round(float(model.intercept_), 2)
r_squared = model.score(xtrain, ytrain)

# Print out the linear equation and r squared value:

print("Linear Equation of Model: y =", coaf, "x +", intercept)
...
for index in range (len(xtest)):
    actual = ytest[index]
    predict_y = predict[index]
    x_coord = xtest[index]
    print("x value:", float(x_coord), "Predicted y value:", predict_y, "Actual y value:", actual)
...
plt.scatter(xtrain, ytrain, c = "green", label = "Training Data")
...
plt.plot(x, coaf*x  + intercept, c = "b", label = "Line of Best Fit")
plt.legend()
plt.show()